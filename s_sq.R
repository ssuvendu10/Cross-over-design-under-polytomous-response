...
tau.1 = NULL
tau.2 = NULL
gamma.sq<- function(x,a){
  g = exp(a + log((x*x*x)-(x*x)+1))/(1 + exp(a + log((x*x*x)-(x*x)+1)))
  return(g)
}
...
pie.A.1 = gamma.sq(1,alpha_a)

pie.A.2 = gamma.sq(2,alpha_a) - gamma.sq(1,alpha_a)
pie.A.3 = 1 - pie.A.1 - pie.A.2

pie.B.1 = gamma.sq(1,alpha_b)
pie.B.2 = gamma.sq(2,alpha_b) - gamma.sq(1,alpha_b)
...
#for log(s)
pie.AB.1 = gamma.sq(1, alpha_b)
pie.AB.2 = gamma.sq(2, alpha_b) - gamma.sq(1, alpha_b)
pie.AB.3 = 1 - pie.AB.1 - pie.AB.2

pie.BA.1 = gamma.sq(1, alpha_a)
pie.BA.2 = gamma.sq(2, alpha_a) - gamma.sq(1, alpha_a)
...
for(i in 1:t){
  ...
  #for stage 2
  alpha_a_hat.1 = log(gamma.est.A.1/(1-gamma.est.A.1)) - log(2)
  alpha_a_hat.2 = log(gamma.est.A.2/(1-gamma.est.A.2)) - log(3)
  alpha_a_hat = (alpha_a_hat.1+alpha_a_hat.2)/2
  
  alpha_b_hat.1 = log(gamma.est.B.1/(1-gamma.est.B.1)) - log(2)
  alpha_b_hat.2 = log(gamma.est.B.2/(1-gamma.est.B.2)) - log(3)
  alpha_b_hat = (alpha_b_hat.1 + alpha_b_hat.2)/2
  
  alpha_hat = (alpha_a_hat + alpha_b_hat)/2
...
prob.test.stat = sum((tau.1>1.645), na.rm = TRUE)/10000
prob.test.stat 
